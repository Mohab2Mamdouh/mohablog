...
abstract class Repository implements RepositoryInterface
{
    ...
    public function query(
        ...
        foreach ($scopes as $scope => $parameters) {
            ...
            if (is_numeric($scope)) {
                if($this->model->hasNamedScope($parameters))
                {
    ...
    public function updateOrCreate(array $attributes, array $values = [], bool $withTrashed = false): ?Model
    {
        ...
        $model = $this->model::query()->when($withTrashed, fn($query) => $query->withTrashed())->updateOrCreate($attributes, $values);
    ...
    {
        $model = $modelId instanceof Model ? $modelId : $this->getById($modelId);
